...
const styles = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .header-tag { font-size: 10px; letter-spacing: 0.25em; text-transform: uppercase; color: #5b5b6e; margin-bottom: 8px; }
  .header h1 { font-family: 'Syne', sans-serif; font-size: clamp(28px, 5vw, 42px); font-weight: 800; line-height: 1.05; color: #f0ece4; letter-spacing: -0.02em; }
  .header h1 span { color: #c8f264; }

  .tab-row { display: flex; gap: 4px; margin-bottom: 40px; background: #13131c; border: 1px solid #1e1e2a; border-radius: 8px; padding: 4px; }
  .tab {
    flex: 1; padding: 10px; text-align: center; cursor: pointer;
    border-radius: 6px; font-family: 'Syne', sans-serif; font-size: 13px;
    font-weight: 600; color: #5b5b6e; transition: all 0.15s; border: none; background: none;
  }
  .tab.active { background: #c8f264; color: #0a0a0f; }
  .tab:hover:not(.active) { color: #c8c4bc; }

  .section { margin-bottom: 40px; }
  .section-label { font-size: 10px; letter-spacing: 0.2em; text-transform: uppercase; color: #5b5b6e; margin-bottom: 16px; display: flex; align-items: center; gap: 10px; }
  .section-label::after { content: ''; flex: 1; height: 1px; background: #1e1e2a; }

  /* Add mode toggle */
  .add-mode-toggle { display: flex; gap: 4px; margin-bottom: 14px; background: #0a0a0f; border: 1px solid #1e1e2a; border-radius: 6px; padding: 3px; }
  .mode-btn { flex: 1; padding: 8px 12px; border: none; border-radius: 4px; font-family: 'Syne', sans-serif; font-size: 11px; font-weight: 600; cursor: pointer; transition: all 0.15s; color: #5b5b6e; background: none; }
  .mode-btn.active { background: #1e1e2a; color: #c8f264; }
  .mode-btn:hover:not(.active) { color: #c8c4bc; }

  /* Channel/Feed subscription box */
  .sub-box { background: #13131c; border: 1px solid #1e1e2a; border-radius: 8px; padding: 18px; display: flex; flex-direction: column; gap: 12px; }
  ...
  .sub-field-label { font-size: 10px; color: #5b5b6e; letter-spacing: 0.1em; text-transform: uppercase; }
  .sub-options { display: flex; gap: 8px; flex-wrap: wrap; }
  .sub-type-btn { padding: 6px 14px; border: 1px solid #1e1e2a; border-radius: 20px; font-family: 'DM Mono', monospace; font-size: 11px; cursor: pointer; background: #0a0a0f; color: #8888a8; transition: all 0.15s; }
  .sub-type-btn.active { border-color: #c8f264; color: #c8f264; background: #1a1f0a; }
  .sub-hint { font-size: 11px; color: #5b5b6e; line-height: 1.5; padding: 10px 12px; background: #0a0a0f; border-radius: 6px; border-left: 2px solid #1e1e2a; }

  .input-row { display: flex; gap: 10px; }
  .url-input { flex: 1; background: #13131c; border: 1px solid #1e1e2a; border-radius: 6px; padding: 12px 16px; font-family: 'DM Mono', monospace; font-size: 13px; color: #e8e4dc; outline: none; transition: border-color 0.2s; }
  .url-input::placeholder { color: #3a3a4e; }
  .url-input:focus { border-color: #c8f264; }

  .date-input { background: #13131c; border: 1px solid #1e1e2a; border-radius: 6px; padding: 12px 14px; font-family: 'DM Mono', monospace; font-size: 13px; color: #e8e4dc; outline: none; transition: border-color 0.2s; color-scheme: dark; }
  .date-input:focus { border-color: #c8f264; }

  .text-input { width: 100%; background: #13131c; border: 1px solid #1e1e2a; border-radius: 6px; padding: 12px 16px; font-family: 'DM Mono', monospace; font-size: 13px; color: #e8e4dc; outline: none; transition: border-color 0.2s; }
  .text-input::placeholder { color: #3a3a4e; }
  ...
  .btn-refresh { background: #1e1e2a; color: #c8c4bc; border: 1px solid #2a2a3a; border-radius: 6px; padding: 8px 14px; font-family: 'Syne', sans-serif; font-size: 11px; font-weight: 600; cursor: pointer; transition: all 0.15s; display: flex; align-items: center; gap: 6px; }
  .btn-refresh:hover { border-color: #c8f264; color: #c8f264; }

  .link-list { display: flex; flex-direction: column; gap: 8px; margin-top: 12px; }
  .link-item { background: #13131c; border: 1px solid #1e1e2a; border-radius: 6px; padding: 10px 14px; display: flex; align-items: center; gap: 12px; animation: fadeIn 0.2s ease; }
  .link-item.is-sub { border-left: 2px solid #c8f264; }

  @keyframes fadeIn { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: translateY(0); } }

  .link-icon { font-size: 16px; flex-shrink: 0; }
  .link-info { flex: 1; overflow: hidden; }
  .link-url { font-size: 12px; color: #8888a8; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .link-meta { font-size: 10px; color: #5b5b6e; margin-top: 2px; }
  .link-badge { display: inline-flex; align-items: center; gap: 4px; background: #1a1f0a; border: 1px solid #c8f264; border-radius: 10px; padding: 2px 8px; font-size: 9px; color: #c8f264; letter-spacing: 0.05em; margin-left: 6px; }
  .btn-remove { background: none; border: none; color: #3a3a4e; cursor: pointer; font-size: 14px; padding: 2px 4px; border-radius: 4px; transition: color 0.15s, background 0.15s; flex-shrink: 0; }
  .btn-remove:hover { color: #ff6b6b; background: #1e1e2a; }

  /* Auto-update toggle */
  .auto-row { display: flex; align-items: center; gap: 12px; padding: 10px 14px; background: #13131c; border: 1px solid #1e1e2a; border-radius: 6px; margin-top: 10px; }
  .toggle { position: relative; width: 36px; height: 20px; flex-shrink: 0; }
  .toggle input { opacity: 0; width: 0; height: 0; }
  .toggle-slider { position: absolute; cursor: pointer; top: 0; left: 0; right: 0; bottom: 0; background: #1e1e2a; border-radius: 20px; transition: 0.2s; border: 1px solid #2a2a3a; }
  .toggle-slider:before { position: absolute; content: ""; height: 14px; width: 14px; left: 2px; bottom: 2px; background: #5b5b6e; border-radius: 50%; transition: 0.2s; }
  .toggle input:checked + .toggle-slider { background: #1a1f0a; border-color: #c8f264; }
  .toggle input:checked + .toggle-slider:before { transform: translateX(16px); background: #c8f264; }
  .auto-label { font-size: 12px; color: #8888a8; flex: 1; }
  .auto-label strong { color: #c8c4bc; font-weight: 500; }

  .option-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: 8px; }
  .option-card { background: #13131c; border: 1px solid #1e1e2a; border-radius: 6px; padding: 14px 12px; cursor: pointer; transition: all 0.15s; text-align: center; }
  .option-card:hover { border-color: #3a3a4e; background: #16161f; }
  .option-card.selected { border-color: #c8f264; background: #1a1f0a; }
  .option-card .opt-icon { font-size: 20px; margin-bottom: 6px; }
  .option-card .opt-label { font-family: 'Syne', sans-serif; font-size: 12px; font-weight: 600; color: #c8c4bc; }
  .option-card.selected .opt-label { color: #c8f264; }
  .option-card .opt-sub { font-size: 10px; color: #5b5b6e; margin-top: 2px; }

  .time-row { display: flex; align-items: center; gap: 16px; margin-top: 16px; flex-wrap: wrap; }
  .time-label { font-size: 11px; color: #5b5b6e; letter-spacing: 0.1em; }
  .time-select { background: #13131c; border: 1px solid #1e1e2a; border-radius: 6px; padding: 8px 14px; font-family: 'DM Mono', monospace; font-size: 13px; color: #e8e4dc; outline: none; cursor: pointer; transition: border-color 0.2s; -webkit-appearance: none; }
  ...
  .delivery-card { background: #13131c; border: 1px solid #1e1e2a; border-radius: 6px; padding: 14px 16px; cursor: pointer; transition: all 0.15s; display: flex; align-items: center; gap: 12px; }
  .delivery-card:hover { border-color: #3a3a4e; }
  .delivery-card.selected { border-color: #c8f264; background: #1a1f0a; }
  .delivery-card .d-icon { font-size: 20px; flex-shrink: 0; }
  .delivery-card .d-info { flex: 1; }
  .delivery-card .d-name { font-family: 'Syne', sans-serif; font-size: 13px; font-weight: 600; color: #c8c4bc; }
  .delivery-card.selected .d-name { color: #c8f264; }
  .delivery-card .d-sub { font-size: 10px; color: #5b5b6e; margin-top: 2px; }
  .delivery-card .d-check { width: 16px; height: 16px; border-radius: 4px; border: 1px solid #3a3a4e; background: transparent; flex-shrink: 0; display: flex; align-items: center; justify-content: center; font-size: 10px; transition: all 0.15s; }
  ...
  .delivery-input-label { font-size: 10px; color: #5b5b6e; letter-spacing: 0.1em; text-transform: uppercase; }
  .delivery-input-field { background: #13131c; border: 1px solid #1e1e2a; border-radius: 6px; padding: 10px 14px; font-family: 'DM Mono', monospace; font-size: 13px; color: #e8e4dc; outline: none; transition: border-color 0.2s; width: 100%; }
  .delivery-input-field::placeholder { color: #3a3a4e; }
  ...
  .style-card { background: #13131c; border: 1px solid #1e1e2a; border-radius: 6px; padding: 16px; cursor: pointer; transition: all 0.15s; }
  .style-card:hover { border-color: #3a3a4e; }
  .style-card.selected { border-color: #c8f264; background: #1a1f0a; }
  .style-card .s-icon { font-size: 22px; margin-bottom: 8px; }
  .style-card .s-name { font-family: 'Syne', sans-serif; font-size: 13px; font-weight: 700; color: #e8e4dc; margin-bottom: 4px; }
  .style-card.selected .s-name { color: #c8f264; }
  .style-card .s-desc { font-size: 11px; color: #5b5b6e; line-height: 1.5; }

  .preview-box { background: #13131c; border: 1px solid #1e1e2a; border-radius: 8px; padding: 24px; position: relative; overflow: hidden; }
  ...
  .preview-loading { display: flex; align-items: center; gap: 10px; color: #5b5b6e; font-size: 12px; }
  .spinner { width: 16px; height: 16px; border: 2px solid #1e1e2a; border-top-color: #c8f264; border-radius: 50%; animation: spin 0.7s linear infinite; flex-shrink: 0; }
  @keyframes spin { to { transform: rotate(360deg); } }
  .preview-meta { font-size: 10px; color: #5b5b6e; letter-spacing: 0.1em; text-transform: uppercase; margin-bottom: 12px; }
  .preview-content { font-size: 13px; color: #c8c4bc; line-height: 1.75; }
  .preview-content strong { color: #e8e4dc; font-weight: 500; }
  .preview-delivery-row { display: flex; gap: 6px; flex-wrap: wrap; margin-top: 14px; padding-top: 14px; border-top: 1px solid #1e1e2a; align-items: center; }
  .preview-badge { display: inline-flex; align-items: center; gap: 5px; background: #16161f; border: 1px solid #2a2a3a; border-radius: 20px; padding: 4px 10px; font-size: 10px; color: #8888a8; }

  .error-box { background: #1a0a0a; border: 1px solid #4a1010; border-radius: 6px; padding: 12px 16px; font-size: 12px; color: #e06060; line-height: 1.5; }
  .warning-note { margin-top: 10px; padding: 10px 14px; background: #1a130a; border: 1px solid #4a3010; border-radius: 6px; font-size: 11px; color: #c8902a; line-height: 1.5; }

  .share-box { background: #13131c; border: 1px solid #1e1e2a; border-radius: 8px; padding: 24px; position: relative; overflow: hidden; }
  ...
  .share-title { font-family: 'Syne', sans-serif; font-size: 15px; font-weight: 700; color: #f0ece4; margin-bottom: 4px; }
  .share-desc { font-size: 12px; color: #5b5b6e; margin-bottom: 20px; line-height: 1.5; }

  .share-link-row { display: flex; gap: 8px; margin-bottom: 16px; }
  .share-link-input { flex: 1; background: #0a0a0f; border: 1px solid #1e1e2a; border-radius: 6px; padding: 10px 14px; font-family: 'DM Mono', monospace; font-size: 11px; color: #8888a8; outline: none; cursor: text; }
  .btn-copy { background: #1e1e2a; color: #c8c4bc; border: none; border-radius: 6px; padding: 10px 16px; font-family: 'Syne', sans-serif; font-size: 12px; font-weight: 600; cursor: pointer; transition: all 0.15s; white-space: nowrap; }
  .btn-copy:hover { background: #2a2a3a; }
  .btn-copy.copied { background: #1a1f0a; color: #c8f264; border: 1px solid #c8f264; }

  .qr-box { display: flex; justify-content: center; margin-top: 16px; padding: 16px; background: #ffffff; border-radius: 8px; }

  .playlist-header { background: #13131c; border: 1px solid #1e1e2a; border-radius: 8px; padding: 20px 24px; margin-bottom: 24px; position: relative; overflow: hidden; }
  ...
  .playlist-header-title { font-family: 'Syne', sans-serif; font-size: 20px; font-weight: 800; color: #f0ece4; margin-bottom: 6px; }
  .playlist-header-desc { font-size: 12px; color: #8888a8; line-height: 1.6; }
  .playlist-header-meta { font-size: 10px; color: #5b5b6e; margin-top: 10px; letter-spacing: 0.1em; text-transform: uppercase; }
  ...
  .btn-secondary { background: transparent; color: #8888a8; border: 1px solid #1e1e2a; border-radius: 6px; padding: 14px 28px; font-family: 'Syne', sans-serif; font-size: 14px; font-weight: 600; cursor: pointer; transition: all 0.15s; }
  .btn-secondary:hover { border-color: #5b5b6e; color: #c8c4bc; }
  .btn-secondary:disabled { opacity: 0.35; cursor: not-allowed; }
  .btn-ghost { background: transparent; color: #5b5b6e; border: 1px solid #1e1e2a; border-radius: 6px; padding: 10px 18px; font-family: 'Syne', sans-serif; font-size: 12px; font-weight: 600; cursor: pointer; transition: all 0.15s; }
  .btn-ghost:hover { border-color: #c8f264; color: #c8f264; }

  .status-pill { display: inline-flex; align-items: center; gap: 6px; background: #1a1f0a; border: 1px solid #c8f264; border-radius: 20px; padding: 6px 14px; font-size: 11px; color: #c8f264; letter-spacing: 0.05em; animation: fadeIn 0.3s ease; }
  .dot { width: 6px; height: 6px; border-radius: 50%; background: #c8f264; }

  .empty-state { padding: 20px; text-align: center; color: #3a3a4e; font-size: 12px; border: 1px dashed #1e1e2a; border-radius: 6px; }
  .divider { height: 1px; background: #1e1e2a; margin: 32px 0; }
`;
...
export default function App() {
  ...
  const addSingleLink = () => {
    ...
    setLinks(prev => [...prev, { id: Date.now(), url: trimmed, type: detectType(trimmed) }]);
    setInputUrl("");
    setPreviewText(null); setPreviewError(null);
  };

  const addSubscription = (sub) => {
    setLinks(prev => [...prev, sub]);
    setPreviewText(null); setPreviewError(null);
  };

  const removeLink = (id) => { setLinks(prev => prev.filter(l => l.id !== id)); setPreviewText(null); };
  ...
  const saveConfig = () => { setSaved(true); setTimeout(() => setSaved(false), 3500); };
          ...
          <>
            <div className="section">
              ...
              <div className="add-mode-toggle">
                <button className={`mode-btn${addMode === "single" ? " active" : ""}`} onClick={() => setAddMode("single")}>
                  🔗 Single Link
                </button>
                <button className={`mode-btn${addMode === "subscribe" ? " active" : ""}`} onClick={() => setAddMode("subscribe")}>
              ...
              {addMode === "subscribe" && (
                <SubscribeInput onAdd={addSubscription} />
              )}
              ...
              {links.length === 0
                ...
                : (
                  <>
                    ...
                    {/* Auto-update toggle — only show if there are subscriptions */}
                    {hasSubscriptions && (
                      <div className="auto-row">
                        ...
                        <div className="auto-label">
                          <strong>Auto-update</strong> — new content from subscribed channels is added automatically each digest cycle
                        </div>
                        <button className="btn-refresh" title="Refresh now">↻ Refresh</button>
                      </div>
                    )}
                  </>
                )
            ...
            <div className="section">
              ...
              <div className="time-row">
                ...
                <select className="time-select" value={deliveryTime} onChange={e => setDeliveryTime(e.target.value)}>
            ...
            <div className="section">
              ...
              {(needsEmail || needsWhatsapp) && (
                <div className="delivery-inputs-row">
                  {needsEmail && <div className="delivery-input-block"><div className="delivery-input-label">✉️ Email address</div><input className="delivery-input-field" type="email" placeholder="you@example.com" value={emailAddress} onChange={e => setEmailAddress(e.target.value)} /></div>}
                  {needsWhatsapp && <div className="delivery-input-block"><div className="delivery-input-label">💬 WhatsApp number (incl. country code)</div><input className="delivery-input-field" type="tel" placeholder="+49 176 12345678" value={whatsappNumber} onChange={e => setWhatsappNumber(e.target.value)} /></div>}
                </div>
              )}
            ...
            <div className="section">
              ...
              <div className="style-grid">
                {STYLES.map(s => (
                  <div key={s.id} className={`style-card${summaryStyle === s.id ? " selected" : ""}`} onClick={() => { setSummaryStyle(s.id); setPreviewText(null); }}>
            ...
            {links.length > 0 && (
              <div className="section">
                ...
                <div className="preview-box">
                  {!previewText && !previewing && !previewError && <div style={{ color: "#3a3a4e", fontSize: 12, textAlign: "center", padding: "12px 0" }}>Click "Preview" to generate a real AI summary</div>}
                  ...
                  {previewText && !previewing && (
                    <>
                      ...
                      <div className="preview-delivery-row">
                        <span style={{ fontSize: 10, color: "#5b5b6e", marginRight: 4 }}>DELIVERED VIA</span>
            ...
            <div className="section">
              ...
              {saved && <div style={{ marginTop: 14 }}><div className="status-pill"><div className="dot" />Saved · {SCHEDULES.find(s => s.id === schedule)?.label} at {deliveryTime} · {deliveryMethods.map(id => DELIVERY_METHODS.find(d => d.id === id)?.name).join(" + ")} · {STYLES.find(s => s.id === summaryStyle)?.name}</div></div>}
            </div>
          </>
        )}
          ...
          <>
            <div className="section">
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <textarea className="text-input" placeholder="Short description (optional) — what's this collection about?" value={playlistDesc} onChange={e => { setPlaylistDesc(e.target.value); setShareUrl(""); }} />
              </div>
            </div>
            ...
            <div className="section">
              <div className="cta-row" style={{ marginBottom: 20 }}>
                <button className="btn-primary" onClick={generateShareLink} disabled={links.length === 0 || !playlistTitle.trim()}>
                  🔗 Generate Share Link
                </button>
              </div>
              ...
              {shareUrl && (
                <div className="share-box">
                  ...
                  <div style={{ marginTop: 16, fontSize: 11, color: "#5b5b6e", lineHeight: 1.6 }}>
                    Anyone with this link can view your playlist, add their own links, and generate their own AI summary in their preferred style.
                  </div>
                </div>
              )}
            </div>
